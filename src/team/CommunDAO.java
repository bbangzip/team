...
public class CommunDAO extends DBConnection{
	...
	//해당 페이지 레코드 선택
		...
		public ListVO getLiData(String id, int num){
			ListVO lv = new ListVO();
			try{
					String sql = "select hdname,subject,content,listpwd,writedate,location,dong,file1,file2,file3 from listTbl where userId=? and num=?";
					dbConn(sql);
					pstmt.setString(1, id);
					pstmt.setInt(2, num);
					rs = pstmt.executeQuery();
		...
		public int update(MemberVO mv){
			...
			try{
				...
				System.out.println(mv.getMl1()+"++"+mv.getMl2()+"++"+mv.getMl3()+"++"+mv.getEmail());
